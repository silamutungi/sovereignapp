...
**TS2801: always await supabase.auth.getSession()**
supabase.auth.getSession() returns a Promise. Checking it without await (e.g. \`if (supabase.auth.getSession())\`) is always truthy and causes a tsc lint_or_type_error (TS2801) that fails the Vercel build. Always use:
