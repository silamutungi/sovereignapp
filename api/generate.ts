      ...
      tools: [
        {
          ...
          input_schema: {
            ...
            properties: {
              ...
              supabaseSchema: {
                type: 'string',
                description: "CRITICAL: Prefix ALL table names with the app name slug to avoid collisions. Example: for app 'WEIR', use 'weir_profiles', 'weir_detections', 'weir_licenses' etc. Never use generic names like 'profiles', 'users', 'licenses', 'earnings', 'matches' without an app-specific prefix. The app name slug is the appName field lowercased with spaces replaced by underscores. This is a hard requirement — unprefixed table names will collide with other apps sharing the same database. CRITICAL: Every CREATE TABLE must use IF NOT EXISTS — CREATE TABLE IF NOT EXISTS table_name (...). Never omit IF NOT EXISTS. Multiple builds share the same database instance. CRITICAL TYPESCRIPT RULE: Because no Supabase type definitions are generated, all Supabase client calls MUST use explicit 'as any' casting to prevent TypeScript inferring 'never' types. Always write: supabase.from('table_name').select('*') as any; supabase.from('table_name').insert({...} as any); supabase.from('table_name').update({...} as any).eq('id', id). Never write supabase.from('table_name') without 'as any' casting. Apply this to every single Supabase query in every file. GENERATE THIS LAST — after all files are complete. Complete Supabase SQL schema. Includes CREATE TABLE, ALTER TABLE ENABLE ROW LEVEL SECURITY, CREATE POLICY for all operations, and CREATE INDEX. Use auth.uid() = user_id for user-owned data. Use standard SQL compatible with PostgreSQL 15.",
              },
            },
